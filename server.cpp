...
int main()
{
	...
	server_address.sin_addr.s_addr = inet_addr("10.6.1.3");
	...
	cout << "Server listening on 10.6.1.2:" << PORT << std::endl;
	...
	while ((received_bytes = recv(client_socket, buffer, sizeof(buffer), 0)) > 0)
	{
		...
		cout << "Received: " << client_message << std::endl;

		key = "0F1E2D3C4B5A6978";
		fixKey(key);

		hex_decryptedText = "";
		for (i=0; i<client_message.length(); i+=16)
		{
			message = client_message.substr(i,16);
		...
		cout << "Padded Response    : " << hex_response << "\n";
		cout << "Key                : " << key << "\n";

		cipherText = "";
		for (i=0; i<hex_response.length(); i+=16)
		{
			message = hex_response.substr(i,16);
